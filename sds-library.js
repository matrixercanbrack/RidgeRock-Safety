...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // IMPORTANT:
  // Your GitHub folder is named "SDS" with capital letters.
  ...
  const completePackageLink = $("#completePackageLink");

  let selectedCategory = "";

  $("#sdsTotal").textContent = sheets.length;

  $("#sdsLastUpdated").textContent =
    `Last updated: ${data.lastUpdated}`;

  if (completePackageLink && data.completePackage?.fileName) {
    completePackageLink.href =
      fileURL(data.completePackage.fileName);
  }

  const categoryCounts = sheets.reduce((counts, sheet) => {
    counts[sheet.category] =
      (counts[sheet.category] || 0) + 1;
  ...
  function renderCategories() {
    ...
    categoryGrid
      .querySelectorAll(".sds-category-card")
      .forEach(button => {
    ...
    return `
      <article class="sds-card">
        ...
        <div class="sds-card-top">
          ...
          <span class="sds-card-number">
            ${String(sheet.number).padStart(2, "0")}
          </span>

          <span class="sds-card-category">
            ${escapeHTML(sheet.category)}
          </span>
  ...
  function renderResults() {
    ...
    if (selectedCategory && query) {
      $("#sdsResultsEyebrow").textContent =
        "Filtered Results";

      $("#sdsResultsTitle").textContent =
        selectedCategory;
